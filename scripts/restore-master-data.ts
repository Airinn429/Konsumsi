...
async function restoreMasterData() {
  console.log("🔄 Memulihkan master data...\n");

  try {
    // Hapus master data lama (TIDAK menghapus user, order, orderItem)
    console.log("🗑️  Menghapus master data lama...");
    await prisma.approver.deleteMany();
    await prisma.sesiWaktu.deleteMany();
    await prisma.satuan.deleteMany();
    await prisma.jenisKonsumsi.deleteMany();
    await prisma.jenisKegiatan.deleteMany();
    await prisma.lokasi.deleteMany();
    await prisma.bagian.deleteMany();
    console.log("   ✅ Master data lama berhasil dihapus\n");

    // Insert master data baru
    console.log("📋 Menambahkan master data...");
    
    console.log("   - Bagian...");
    for (const item of bagianOptions) {
      await prisma.bagian.create({ data: { name: item } });
    }

    console.log("   - Lokasi...");
    for (const item of lokasiOptions) {
      await prisma.lokasi.create({ data: { name: item } });
    }

    console.log("   - Jenis Kegiatan...");
    for (const item of kegiatanOptions) {
      await prisma.jenisKegiatan.create({ data: { name: item } });
    }

    console.log("   - Jenis Konsumsi (Tipe Tamu)...");
    for (const item of tipeTamuOptions) {
      await prisma.jenisKonsumsi.create({ data: { name: item } });
    }

    console.log("   - Satuan...");
    for (const item of satuanOptions) {
      await prisma.satuan.create({ data: { name: item } });
    }

    console.log("   - Sesi Waktu...");
    for (const item of sesiWaktuOptions) {
      await prisma.sesiWaktu.create({ data: { name: item } });
    }

    console.log("   - Approver...");
    for (const item of approverOptions) {
      await prisma.approver.create({ data: { name: item } });
    }

    console.log("\n✅ Master data berhasil dipulihkan!");
    console.log("📊 Data user, order, dan consumption item tetap aman!\n");

  } catch (error) {
    console.error("❌ Error saat memulihkan master data:", error);
    process.exit(1);
  } finally {
