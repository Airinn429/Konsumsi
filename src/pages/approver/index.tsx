...
const OrderFormContent: React.FC<OrderFormProps> = ({ initialData, onSubmit, onCancel, isSuccessful, setIsSuccessful }) => {
    ...
    // 4. Logic Validasi Tanggal Pengiriman (H+1 dari Tanggal Permintaan)
    const minDeliveryDateObj = useMemo(() => {
        const minDate = new Date(formData.tanggalPermintaan);
        minDate.setDate(minDate.getDate()); // Mulai dari tanggal permintaan
    ...
    const handleDateChange = (name: keyof FormData, date: Date | undefined) => { 
        ...
        if (name === 'tanggalPermintaan' && date) {
            ...
            minDeliveryDate.setDate(minDeliveryDate.getDate()); // Minimum hari yang sama
            ...
            // Jika tanggal pengiriman yang sudah dipilih kurang dari hari ini (Tanggal Permintaan), set ke hari ini
    ...
    return (
        <div className="container mx-auto p-4 sm:p-6 md:p-8">
            ...
            <Dialog open={isFormVisible} onOpenChange={(isOpen : boolean) => {
                ...
                <DialogContent className="p-0 border-0 bg-transparent shadow-none data-[state=open]:bg-transparent sm:rounded-lg h-full max-w-full sm:max-w-3xl"> {/* PERBAIKAN: Tambahkan h-full max-w-full sm:max-w-3xl */}
                    ...
                    />
                </DialogContent> {/* Tag penutup untuk DialogContent */}
            </Dialog> {/* Tag penutup untuk Dialog */}
        </div>
    );
}